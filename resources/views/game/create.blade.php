<x-app-layout>
    ...
    <div class="container max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <section class="grid items-start gap-8">
            <form class="w-full mx-auto" method="POST" action="{{ route('movies.store') }}">
                @csrf
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    >
                        ...
                        @foreach ($types as $genre)
                            <option value="{{ $genre->id_genre }}">{{ ucfirst($genre->nom) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    >
                        ...
                        @foreach ($distributors as $distributeur)
                            <option value="{{ $distributeur->id_distributeur }}">{{ ucfirst($distributeur->nom) }} -
                ...
                <div class="mb-5">
                    <button type="submit"
                        class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Add</button>
                </div>
            </form>
        </section>
    </div>
